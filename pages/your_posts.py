...
if not user_posts:
    ...
    for index, post in enumerate(posts):
        if post["Name"] == username:
            ...
            with st.expander("✏️ Edit Post"):
                new_title = st.text_input(f"Edit Title {index}", value=post["Title"], key=f"title_{index}")
                new_content = st.text_area(f"Edit Content {index}", value=post["Content"], height=200, key=f"content_{index}")
                
                # Unique key for the save button
                if st.button(f"💾 Save Changes", key=f"save_{index}"):
                    edit_post(index, new_title, new_content)
                    st.success("✅ Post updated successfully!")
                    st.rerun()

            # Unique key for the delete button
            if st.button(f"🗑️ Delete Post", key=f"delete_{index}"):
                delete_post(index)
                st.warning("❌ Post deleted!")
                st.rerun()
